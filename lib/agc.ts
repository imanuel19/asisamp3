...
class AGC {
  ...
  async getSearch(query: string): Promise<Song[]> {
    ...
    if (!items) {
      try {
        const searchUrl = `https://v0-dfdfde.vercel.app/api/search?q=${q}`
  ...
  async getDownload(id: string): Promise<Song | null> {
    ...
    if (!data) {
      try {
        const infoUrl = `https://v0-dfdfde.vercel.app/api/search?q=${id}`
    ...
    if (!items) {
      try {
        ...
        if (data) {
          ...
          const searchUrl = `https://v0-dfdfde.vercel.app/api/search?q=${encodeURIComponent(title)}`
          const response = await fetch(searchUrl)
